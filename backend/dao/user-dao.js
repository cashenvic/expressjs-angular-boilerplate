...
async function getByUsername(username) {
  ...
  return models.User.findOne({
    ...
    return {
      status: 'error',
      message:
        `Une erreur est survenue lors de la récupération de l'utilisateur avec le nom d'utilisateur ` +
        username,
      details: err,
    };
  });
...
async function update(criteria, username) {
  ...
  return models.User.update(criteria, { where: { username: username } }).catch(
    (err) => {
      console.error(err);
      return {
        status: 'error',
...
async function getById(id) {
  return models.User.findByPk(id).catch((err) => {
    return {
      status: 'error',
      message:
        `Une erreur est survenue lors de la récupération de l'utilisateur avec l'identifiant ` +
        id,
      details: err,
    };
  });
